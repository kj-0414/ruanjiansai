    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <span class="user-info" @click="toggleUserMenu">
          ...
          <el-tag size="small" class="role-tag">求职者</el-tag>
...
import {
  ArrowDown, House, Document, PieChart, Briefcase, Message,
...
const menuItems = computed(() => [
  { path: '/job-seeker/home', label: '首页', icon: House },
  { path: '/job-seeker/resume', label: '我的简历', icon: Document },
  { path: '/job-seeker/ability-analysis', label: '能力分析', icon: PieChart },
  { path: '/job-seeker/job-match', label: '岗位匹配', icon: Briefcase },
  { path: '/job-seeker/messages', label: '消息中心', icon: Message }
])

const isActive = (path) => {
  if (path === '/job-seeker/home') {
    return route.path === '/job-seeker/home'
  }
  return route.path.startsWith(path)
}

const handleNavClick = (item) => {
  console.log("[JobSeekerLayout] === 菜单点击调试 ===")
  console.log("[JobSeekerLayout] 点击的菜单路径:", item.path)
  console.log("[JobSeekerLayout] 当前路由:", router.currentRoute.value.fullPath)
  console.log("[JobSeekerLayout] 当前角色:", authService.getCurrentRole())
  console.log("[JobSeekerLayout] 是否已认证:", authService.isAuthenticated())
